...
class on_raw_reaction_remove(commands.Cog):
    ...
    async def on_raw_reaction_remove(self, playload : discord.RawReactionActionEvent):
        ...
        if not role:
            return
        if role.id not in [member_role.id for member_role in member.roles]:
            return

        try: await member.remove_roles(role, reason = "React-Role")
        except: pass


def setup(bot):
    bot.add_cog(on_raw_reaction_remove(bot))